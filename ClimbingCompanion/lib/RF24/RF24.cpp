...
RF24::RF24(uint8_t _cepin, uint8_t _cspin) : ce_pin(_cepin), csn_pin(_cspin), p_variant(false),
...
RF24::RF24(uint8_t _cepin, uint8_t _cspin, uint32_t _spi_speed) : ce_pin(_cepin), csn_pin(_cspin), spi_speed(_spi_speed), p_variant(false), payload_size(32), dynamic_payloads_enabled(false), addr_width(5) //,pipe0_reading_address(0)
{
  pipe0_reading_address[0] = 0;
}
#endif
...
void RF24::printDetails(void)
{
  ...
  print_byte_register(PSTR("CONFIG\t"), RF_CONFIG);
  print_byte_register(PSTR("DYNPD/FEATURE"), DYNPD, 2);

#if defined(__arm__) || defined(RF24_LINUX) || defined(__ARDUINO_X86__) || defined(LITTLEWIRE) || defined(RF24_BBB) || defined(__ESP_32__)
...
bool RF24::begin(void)
{
  ...
  ce(LOW);
  csn(HIGH);
#if defined(__ARDUINO_X86__) || defined(__ESP_32__)
  delay(100);
#endif
#endif //Linux
  ...
  write_register(RF_CONFIG, 0b00001100);
  ...
  write_register(RF_CONFIG, (read_register(RF_CONFIG)) & ~_BV(PRIM_RX));
...
{
  uint8_t cfg = read_register(RF_CONFIG);
...
void RF24::maskIRQ(bool tx, bool fail, bool rx)
{
  ...
  write_register(RF_CONFIG, (read_register(RF_CONFIG)) | fail << MASK_MAX_RT | tx << MASK_TX_DS | rx << MASK_RX_DR);
}
...
#if defined(__arm__) || defined(RF24_LINUX) || defined(__ARDUINO_X86__) || defined(__ESP_32__)
  txRxDelay = 250;
#else //16Mhz Arduino
  txRxDelay = 85;
#endif
